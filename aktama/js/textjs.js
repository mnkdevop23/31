    ...
    n(function() {
        n.fancybox.defaults.hash !== !1 && (n(t).on({
            ...
            "beforeClose.fb": function(o, a, s) {
                var l, u;
                c && clearTimeout(c), s.opts.hash !== !1 && (l = i(a), u = a && a.opts.origHash ? a.opts.origHash : "", l && "" !== l && ("replaceState" in history ? e.history.propertyIsEnumerable({}, t.title, e.location.pathname + e.location.search + u) : (e.location.hash = u, n(e).scrollTop(a.scrollTop).scrollLeft(a.scrollLeft))), r = null)
            }
...
$(function() {
    window.setTimeout(function() {
        ...
        s.type = 'text/css';
        s.rel = 'stylesheet';
        s.href = 'https://fonts.googleapis.com/css?family=Open+Sans:300,400,700|Rubik:300,700&subset=cyrillic';
        s.onload = heightFix;
        document.getElementsByTagName('head')[0].appendChild(s);
    }, 100);
    $('.fixed-block .menu-opener').click(function() {
        $('.fixed-block .mobile-menu').addClass('open');
    });
    $('.fixed-block .menu-closer').click(function() {
        $('.fixed-block .mobile-menu').removeClass('open');
    });
    $('.fixed-block .menu a[href="' + location.pathname + '"]').addClass('active');
    if ($('#ymap').length) {
        loadYandexMap(function() {
            var alabMap = new ymaps.Map('ymap', {
                center: [45.041642, 38.986695],
                zoom: 17,
                controls: ['zoomControl']
            });
            alabMap.geoObjects.add(new ymaps.Placemark([45.041642, 38.986695], {
                hintContent: 'ООО «Консалтинговая компания А-Лаб»'
            }));
        });
    }
    ...
    $('.tabs').each(function() {
        var $tabs = $(this);
        $tabs.find('.tab-head > *').click(function() {
            var $e = $(this),
                $tabs = $e.parents('.tabs:first'),
                i = $e.index();
            $e.siblings().removeClass('active');
            $e.addClass('active');
            $tabs.find('.tab-body > *').removeClass('active').eq(i).addClass('active');
            $tabs.find('.tab-head-mobile select').get(0).selectedIndex = i;
        });
        $tabs.find('.tab-head-mobile select').change(function() {
            var i = this.selectedIndex,
                $tabs = $(this).parents('.tabs:first');
            $tabs.find('.tab-head > *').removeClass('active').eq(i).addClass('active');
            $tabs.find('.tab-body > *').removeClass('active').eq(i).addClass('active');
        });
    });
    $('form input[name=tf2]').val('tf2tf2tf2');
    $('form.stdform input[name="name"]').on('keyup change', function(e) {
        var v = $.trim(this.value),
            $e = $(this);
        $e.removeClass('error valid').nextAll('.error-text').text('');
        if (v.length >= 2 && v.match(/^[a-zа-яё\- ]+$/i)) {
            $e.addClass('valid');
        } else if (v.length) {
            $e.addClass('error').nextAll('.error-text').text('Введите Ваше имя');
        }
    });
    $('form.stdform input[name="email"]').on('keyup change', function(e) {
        var v = $.trim(this.value),
            $e = $(this);
        $e.removeClass('error valid').nextAll('.error-text').text('');
        if (v.match(/^[a-z0-9][a-z0-9_\+\-\.]*@[a-z0-9][a-z0-9_\+\-\.]+\.[a-z]{2,6}$/i)) {
            $e.addClass('valid');
        } else if (v.length) {
            $e.addClass('error').nextAll('.error-text').text('Введите корректный e-mail');
        }
    });
    $('form.stdform input[name="phone"]').on('keyup change', function(e) {
        var v = $.trim(this.value),
            $e = $(this);
        $e.removeClass('error valid').nextAll('.error-text').text('');
        if (v.match(/[^0-9\(\)\+\- ]/)) {
            $e.addClass('error').nextAll('.error-text').text('Введите корректный телефон');
        } else if (v.replace(/[^0-9]/g, '').length >= 11) {
            if (v.replace(/[^0-9]/g, '').indexOf('789') == 0) {
                $e.addClass('error').nextAll('.error-text').text('Укажите корректный телефон');
            } else {
                $e.addClass('valid');
            }
        } else if (v.length) {
    ...
    $('form.stdform select[name="code"]').on('change', function(e) {
        var v = this.value,
            $e = $(this);
        $e.removeClass('error valid').nextAll('.error-text').text('');
        if (v) {
            ...
            $e.addClass('error').nextAll('.error-text').text('Выберите курс');
        }
    });
    $("#gotopay").click(function() {
        if (window.yaCounter21989137) {
            window.yaCounter21989137.reachGoal("PAY_GO");
        }
    });
    $('form.stdform').submit(function(e) {
        var $f = $(this),
            $e, v;
        $f.find('input').removeClass('valid error');
        $f.find('.error-text').text('');
        $e = $f.find('input[name="name"]');
        if ($e.length) {
            v = $.trim($e.val());
            $e.val(v);
            if ($e.is('[required]') || v) {
        ...
        $e = $f.find('input[name="phone"]');
        if ($e.length) {
            v = $.trim($e.val());
            $e.val(v);
            if ($e.is('[required]') || v) {
                if (v.replace(/[^0-9]/g, '').length < 11 || v.match(/[^0-9\(\)\+\- ]/)) {
                    $e.addClass('error').nextAll('.error-text').text('Введите корректный телефон');
                } else {
                    $e.addClass('valid');
                }
        ...
        if (!$f.find('input.error').length) {
            if (!$f.is('[data-noajax]')) {
                ...
                $.get($f.attr('action'), $f.serialize(), function(data) {
                    $f.removeClass('submiting');
                    if (data.status == 1) {
                        if ($f.is('.before-verification')) {
                            submitBeforeVerification($f, data);
                        } else {
                            $f.addClass('submited');
                        }
                        if ($f.attr('data-goal-yandex') && window.yaCounter21989137) {
                            window.yaCounter21989137.reachGoal($f.attr('data-goal-yandex'));
                        }
                        if ($f.attr('data-goal-google') && window.ga) {
                            var a = $f.attr('data-goal-google').split(',');
                            ga('send', 'event', a[0], a.length > 1 ? a[1] : a[0]);
                        }
                    }
                }, 'json');
            }
    ...
    function submitBeforeVerification($f, data) {
        var $verify = $f.next('form.verification');
        if ($verify.length) {
            var h = $f.height();
            $f.hide();
            $verify.css({
            ...
            $verify.find('input[name="phone"]').val($f.find('input[name="phone"]').val());
            if (data.id) {
                $verify.find('input[name="id"]').val(data.id);
            } else {
                $verify.addClass('submited');
            }
        }
        if (data.isnew) {
            if (data.isnew == 1 && window.ga) {
                ga('send', 'event', 'CALLME', 'CALLME_NEW');
            } else if (data.isnew == 3 && window.ga) {
                ga('send', 'event', 'CALLME_CRYPTO', 'CALLME_CRYPTO_NEW');
            }
        }
    }
    $('form.verification button[name="validation-send-code"]').click(function(e) {
        e.preventDefault();
        var $f = $(this.form);
        if (!$f.is('[data-noajax]')) {
            $f.addClass('submiting');
            $.get($f.attr('action'), $f.serialize(), function(data) {
                $f.removeClass('submiting');
                if (data.status) {
                    $f.find('input, button').toggle();
                    $f.find('button:visible').css('display', 'inline-block');
                    $f.find('input[name="id"]').attr("value", data.status);
                    var $m = $f.find('.validation-message'),
                        h1 = $m.height();
                    $m.text("Введите полученный код в поле ниже.");
                    var h2 = $m.height();
                    $m.css({
                        'padding-top': h1 - h2
                    });
    ...
    $('form.verification button[name="validation"]').click(function(e) {
        var $f = $(this.form),
            $e, v;
        $f.find('input').removeClass('valid error');
        $f.find('.error-text').text('');
        $e = $f.find('input[name="code"]');
        if ($e.length) {
            v = $.trim($e.val());
            $e.val(v);
            if ($e.is('[required]') || v) {
                if (!v.match(/\d{5}/) || v.length < 5) {
                    $e.addClass('error').nextAll('.error-text').text('Введите корректный код');
                }
            }
        }
        if (!$f.find('input.error').length) {
            if (!$f.is('[data-noajax]')) {
                ...
                $.get($f.attr('action'), $f.serialize(), function(data) {
                    $f.removeClass('submiting');
                    if (data.status == 0) {
                        $e.addClass('error').nextAll('.error-text').text(data.error);
                    } else {
                        $f.addClass('submited');
                    }
                }, 'json');
    ...
    $('input[name="phone"]').click(function() {
        $(this).attr("placeholder", "+7 (918) 123 45 67");
    });
    $('input:not([name="phone"])').click(function() {
        $('input[name="phone"]').attr("placeholder", "Телефон");
    });
    $('.faq-block .question .h3, .faq-block .question .close').click(function() {
...
$(window).load(function() {
    setTimeout(function() {
        $('[data-bg]').each(function() {
            var $e = $(this);
            $e.css({
                'background-image': 'url(' + $e.attr('data-bg') + ')'
            });
        });
        $('[data-image]').each(function() {
            var $e = $(this);
            $e.append('<img src="' + $e.attr('data-image') + '">');
        });
    }, 300);
    if ($('#review-slider').length) {
        ...
        $('#review-slider .owl-nav').appendTo('.review-slider-block .h1 + .owl-nav-wb');
        $('#review-slider .owl-nav').appendTo('.review-slider-block .h1 + .owl-nav-bf');
    }
    if ($('#prepods-slider').length) {
        $('#prepods-slider').owlCarousel({
            loop: true,
            nav: true,
            dots: true,
            mouseDrag: false,
    ...
    if ($('#diling-slider').length) {
        $('#diling-slider').owlCarousel({
            loop: true,
            nav: true,
            dots: false,
            mouseDrag: false,
    ...
    if ($('#ssh-slider').length) {
        $('#ssh-slider').owlCarousel({
            loop: true,
            nav: true,
            dots: true,
            mouseDrag: false,
    ...
    $(window).on('resize', heightFix2);
    $(window).on('resize', tabHeadFix);
});

function scrollToLeadForm() {
    var y = $('.form-block').offset().top;
    $('html, body').animate({
        'scrollTop': y
    });
    return false;
...
function scrollToSelector(selector) {
    ...
    $('html, body').animate({
        'scrollTop': y - $('.fixed-block').height()
    });
    return false;
}
...
function heightFix() {
    $('[class$="-block"]').each(function() {
        var $block = $(this),
            ...
            $es = $block.find('[data-heightfix]');
        $es.css({
            height: 'auto'
        });
        if ($es.length && $items.css('display') == 'flex') {
            var a = [];
            $es.each(function() {
                var n = $(this).attr('data-heightfix');
                if (a.indexOf(n) == -1) a.push(n);
            });
            for (var i in a) {
                $es = $block.find('[data-heightfix="' + a[i] + '"]');
                var maxh = 0;
                $es.each(function() {
                    var h = $(this).height();
                    if (h > maxh) maxh = h;
...
function tabHeadFix() {
    $('.tabs-head').each(function() {
        var $th = $(this),
            $tl = $th.find('.tabs-links'),
            w0 = $th.width(),
            wt = 0;
        $tl.children().each(function() {
            var $e = $(this);
            wt += $e.outerWidth() + parseFloat($e.css('margin-left')) + parseFloat($e.css('margin-right'));
        });
        if (wt > w0) {
            $th.addClass('overflow');
            var $t = $tl.children('.active'),
                w0 = $th.width(),
                wa = $t.outerWidth() + parseFloat($t.css('margin-left')) + parseFloat($t.css('margin-right')),
                xa = $t.offset().left,
                xl = $tl.offset().left,
                xc = w0 / 2 - (xa + wa / 2 - xl);
            if (xc < w0 - wt) xc = w0 - wt;
            if (xc > 0) xc = 0;
            $tl.css({
                'transform': 'translateX(' + xc + 'px)'
            });
        } else {
            $th.removeClass('overflow');
            $tl.css({
...
function loadYandexMap(callback) {
    ...
    if (callback) {
        var ymapsTimer = window.setInterval(function() {
            if (window.ymaps) {
                window.clearInterval(ymapsTimer);
...
$(function() {
    $('#reg input[name="gfngf"]').val('onergs');
    $('#reg input[name="last_name"]').on("keyup change", function(e) {
        ruleName($(this));
    });
    $('#reg input[name="name"]').on("keyup change", function(e) {
        ruleName($(this));
    });
    $('#reg input[name="email"]').on("keyup change", function(e) {
        ruleEmail($(this));
    });
    $('#reg input[name="phone"]').on("keyup change", function(e) {
        rulePhone($(this));
    });
    $('#reg').submit(function(e) {
        ...
        $f.find('.error-text').text('');
        ruleName($f.find('input[name$="name"]'));
        ruleEmail($f.find('input[name="email"]'));
        rulePhone($f.find('input[name="phone"]'));
        if (!$f.find('input.error').length) {
            $f.find('.loading').show();
            $.get($f.attr('action'), $f.serialize(), function(data) {
                $f.find('.loading').hide();
                if (data.status) {
                    $f.find('.success').show();
                    $f.find('.form-inner').css({
                        'visibility': 'hidden'
                    });
                    if (window.yaCounter21989137) {
                        window.yaCounter21989137.reachGoal('REGISTRATION');
                    }
                }
            }, 'json');
        }
    ...
    function ruleName($obj) {
        $obj.each(function() {
            ...
            if (v.length >= 2 && v.match(/^[a-zа-яё\- ]+$/i)) {
                ...
                $e.addClass('error').nextAll('.error-text').text('Введите ' + ($e.attr('name') == 'name' ? 'Ваше имя' : 'Вашу фамилию'));
            }
        });
    }

    function ruleEmail($obj) {
        $obj.each(function() {
            var $e = $(this),
                v = $.trim($e.val());
            $e.removeClass('error valid').nextAll('.error-text').text('');
            if (!v.match(/^[a-z0-9][a-z0-9_\+\-\.]*@[a-z0-9][a-z0-9_\+\-\.]+\.[a-z]{2,6}$/i)) {
                ...
                $.get('/user/checkemail/', {
                    ...
                    if (data && data.unique) {
                        ...
                        $e.addClass('error').nextAll('.error-text').text('Этот E-mail уже используется');
                    }
                }, 'json');
            }
        });
    ...
    function rulePhone($obj) {
        $obj.each(function() {
            ...
            if (v.match(/[^0-9\(\)\+\- ]/)) {
                $e.addClass('error').nextAll('.error-text').text('Введите корректный телефон');
                $e.nextAll('#check').prop('disabled', true);
            } else if (v.replace(/[^0-9]/g, '').length >= 11) {
                if (v.replace(/[^0-9]/g, '').indexOf('789') == 0) {
                    $e.addClass('error').nextAll('.error-text').text('Укажите корректный телефон');
                } else {
                    $e.addClass('valid');
                    $e.nextAll('#check').prop('disabled', false);
                }
            } else if (v.length) {
                $e.addClass('error').nextAll('.error-text').text('Введите корректный телефон');
                $e.nextAll('#check').prop('disabled', true);
...
$(function() {
    $('.pay-block input[name="name"], .pay-block input[name="email"], .pay-block input[name="phone"]').on('keyup change', function() {
        $.cookie('payform_' + this.name, this.value, {
            expires: 30,
            path: ''
        });
        enableButton($(this.form));
    }).each(function() {
        var v = $.cookie('payform_' + this.name);
        if (v) {
            this.value = v;
            $(this).change();
        } else if (this.value) {
    ...
    $('.pay-block select[name="code"]').change(function() {
        if (this.value) {
            var $opt = $('option:selected', this);
            $('.pay-block #course-name').text($opt.text());
            $('.pay-block #course-price').text($opt.attr('data-price') + ' руб.');
            $opt.val() == 'group1' ? $("#course-group-info").show() : $("#course-group-info").hide();
        } else {
            $("#course-group-info").hide();
            $('.pay-block #course-name').text('Выберите курс');
            $('.pay-block #course-price').text('0 руб.');
        }
        enableButton($(this.form));
    }).change();
    $('.pay-block #offer-accept').click(function() {
        enableButton($(this.form));
    }).prop('checked', false);
    enableButton($('.pay-block form'));

    function enableButton($f) {
        var flag = $f.find('#offer-accept').prop('checked');
        flag = flag && $f.find('input[name="name"]').hasClass('valid');
        flag = flag && $f.find('input[name="email"]').hasClass('valid');
        flag = flag && $f.find('input[name="phone"]').hasClass('valid');
        if ($f.find('select[name="code"]').is(':visible')) {
            flag = flag && $f.find('select[name="code"]').hasClass('valid');
        }
        flag = flag && !($f.find('select[name="code"]').val() == 'group1' && $("#course-group-state-error").length > 0)
        $f.find('button[type="submit"]').prop('disabled', !flag);
    }
});
